...
1. 读取工作区配置。
2. 扫描工作区文件，并跳过已被 `exclude` 规则整体覆盖的目录。
...
`src/fileScanner.ts` 递归扫描工作区中的普通文件，并统一输出为使用 `/` 的相对路径列表。

扫描前会先把 `exclude` 规则归一化，并在进入子目录前判断该目录是否已被这些规则整体覆盖；如果匹配，就直接跳过整棵子树，不再继续读取其中的文件。
...
- 目录折叠逻辑可以直接基于相对路径树工作
- `node_modules` 这类大目录不会被无意义地完整遍历

同时会保留 `.vscode/settings.json` 所在目录的扫描，避免扩展把自己的工作区配置文件跳过。
